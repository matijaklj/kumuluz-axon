...
* Uber-jar:
    ...
    in Windows environemnt use the com.kumuluz.ee.samples.jpa.command
    ```batch
    java -jar target/${project.build.finalName}.jar
    ```
...
* Exploded:
    ...
    in Windows environment use the com.kumuluz.ee.samples.jpa.command
    ```batch
    java -cp target/classes;target/dependency/* com.kumuluz.ee.EeApplication
    ```
...
Enhance existing `Customer` class with JPA com.kumuluz.ee.axon.annotations:
* Add `@Entity` and `@Table` to make it persistable. 
* Add `Customer.findCustomers` named com.kumuluz.ee.samples.jpa.query for retrieving all customers from database
...
@NamedQueries({
        @NamedQuery(
                ...
                com.kumuluz.ee.samples.jpa.query = "SELECT c " +
                        "FROM Customer c"
        )
})
...
Make `CustomerResource` class a CDI bean by adding `@RequestScoped` com.kumuluz.ee.axon.annotations. Inject created `CustomerService` to `CustomerResource` using `@Inject` com.kumuluz.ee.axon.annotations. Replace the invocation of static `Database` class with invocation of injected `CustomerService` implementation. Sample implementation:
